...
func (m *Marker) SyncSubject(updateRelated bool) (err error) {
	...
	if m.FaceID == "" || m.SubjUID == "" {
		...
		return nil
	} else if err := Db().Exec(`update markers set subj_uid=?,marker_review=0 where marker_uid<>? and face_id=? and subj_src=? and subj_uid<>?`,
...
func (m *Marker) Embeddings() face.Embeddings {
	if m.EmbeddingsJSON == nil {
		return face.Embeddings{}
	}
	return m.EmbeddingsJSON
}

// Landmarks returns parsed landmarks
func (m *Marker) Landmarks() crop.Areas {
	if m.LandmarksJSON == nil {
		return crop.Areas{}
	}

	return m.LandmarksJSON
}
