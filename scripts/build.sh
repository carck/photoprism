...
if [[ $1 == "debug" ]]; then
  ...
  BUILD_CMD=("$GO_BIN" build -tags NOTENSORFLOW -ldflags "-extldflags \"-Wl,-rpath -Wl,\$ORIGIN/../lib\" -s -w -X main.version=${BUILD_ID}" -o "${BUILD_NAME}" cmd/photoprism/photoprism.go)
fi
